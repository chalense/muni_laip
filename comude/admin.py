...
@admin.register(NumeralComude)
class NumeralComudeAdmin(admin.ModelAdmin):
    """Admin para Numerales de COMUDE"""
    list_display = [
        'codigo_badge',
        'titulo_corto',
        'activo_badge',
...
@admin.register(CarpetaComude)
class CarpetaComudeAdmin(admin.ModelAdmin):
    """Admin para Carpetas de COMUDE"""
    list_display = [
        'icono_nivel',
        'nombre_jerarquico',
        'numeral_link',
    ...
    def numeral_link(self, obj):
        url = reverse('admin:comude_numeralcomude_change', args=[obj.numeral.pk])
        return format_html('<a href="{}" style="color: #10B981;">📋 {}</a>', url, obj.numeral.codigo)
...
@admin.register(DocumentoComude)
class DocumentoComudeAdmin(admin.ModelAdmin):
    """Admin para Documentos de COMUDE"""
        ...
        ('Ubicación', {
            ...
            'description': '''
                <div style="background: #D1FAE5; border-left: 4px solid #10B981; padding: 12px; margin: 10px 0; border-radius: 4px;">
                    <strong style="color: #065F46;">💡 Importante sobre la ubicación:</strong>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; color: #047857;">
                        ...
                        <li><strong>Crear carpetas:</strong> Ve a <a href="/admin/comude/carpetacomude/add/" style="color: #10B981; font-weight: 600;">Carpetas > Agregar carpeta</a>.</li>
                    </ul>
                </div>
            '''
        }),
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == "carpeta":
            ...
            if obj_id:
                try:
                    documento = DocumentoComude.objects.get(pk=obj_id)
                    kwargs["queryset"] = CarpetaComude.objects.filter(
                        numeral=documento.numeral
                    ).select_related('padre')
                except DocumentoComude.DoesNotExist:
                    kwargs["queryset"] = CarpetaComude.objects.none()
            else:
        ...
        if 'carpeta' in form.base_fields:
            ...
            if obj and obj.numeral:
                form.base_fields['carpeta'].help_text = f'''
                    <span style="color: #059669; font-weight: 600;">📁 Carpetas del Numeral {obj.numeral.codigo}</span><br>
    ...
    def numeral_link(self, obj):
        url = reverse('admin:comude_numeralcomude_change', args=[obj.numeral.pk])
        return format_html('<a href="{}" style="color: #10B981; font-weight: 600;">{}</a>', url, obj.numeral.codigo)
    numeral_link.short_description = 'Numeral'
    
    def carpeta_ruta(self, obj):
        if obj.carpeta:
            return format_html(
                '<span style="color: #059669; font-size: 12px; font-weight: 600;">📁 {}</span>',
                obj.carpeta.get_ruta_completa()
            )
        return format_html(
            '<span style="color: #10B981; font-size: 12px; font-weight: 600; '
            'background: #D1FAE5; padding: 2px 8px; border-radius: 4px;">📋 Raíz del Numeral</span>'
        )
    ...
    def acciones_rapidas(self, obj):
        ...
        editar_url = reverse('admin:comude_documentocomude_change', args=[obj.pk])
        eliminar_url = reverse('admin:comude_documentocomude_delete', args=[obj.pk])
