...
class NumeralComude(models.Model):
    """
    Numerales específicos de COMUDE (Consejo Municipal de Desarrollo)
    """
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = slugify(f"comude-{self.codigo}")
        if self.orden == 0:
            # Extraer número del código (ej: "01A" -> 1)
            try:
    ...
    def get_absolute_url(self):
        return reverse('comude:numeral_detail', kwargs={'slug': self.slug})
...
class CarpetaComude(models.Model):
    """
    Estructura jerárquica de carpetas para COMUDE
    """
...
def path_documento_comude(instance, filename):
    """
    Genera la ruta para documentos de COMUDE
    """
    nombre_limpio = filename.replace(' ', '_')
    ruta_base = f'comude/numeral_{instance.numeral.codigo}/'
...
class DocumentoComude(models.Model):
    """
    Documentos de COMUDE
    """
    ...
    archivo = models.FileField(
        upload_to=path_documento_comude,
