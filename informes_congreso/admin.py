...
@admin.register(NumeralInformesCongreso)
class NumeralInformesCongresoAdmin(admin.ModelAdmin):
    """Admin para Numerales de Informes al Congreso"""
    list_display = [
        'codigo_badge',
        'titulo_corto',
        'activo_badge',
...
@admin.register(CarpetaInformesCongreso)
class CarpetaInformesCongresoAdmin(admin.ModelAdmin):
    """Admin para Carpetas de Informes al Congreso"""
    list_display = [
        'icono_nivel',
        'nombre_jerarquico',
        'numeral_link',
    ...
    def numeral_link(self, obj):
        url = reverse('admin:informes_congreso_numeralinformescongreso_change', args=[obj.numeral.pk])
        return format_html('<a href="{}" style="color: #8B5CF6;">📋 {}</a>', url, obj.numeral.codigo)
    numeral_link.short_description = 'Numeral'
    
    def total_docs_badge(self, obj):
        total = obj.total_documentos()
        color = '#8B5CF6' if total > 0 else '#D1D5DB'
...
@admin.register(DocumentoInformesCongreso)
class DocumentoInformesCongresoAdmin(admin.ModelAdmin):
    """Admin para Documentos de Informes al Congreso"""
    ...
    fieldsets = (
        ...
        ('Ubicación', {
            ...
            'description': '''
                <div style="background: #EDE9FE; border-left: 4px solid #8B5CF6; padding: 12px; margin: 10px 0; border-radius: 4px;">
                    <strong style="color: #5B21B6;">💡 Importante sobre la ubicación:</strong>
                    <ul style="margin: 8px 0 0 0; padding-left: 20px; color: #6D28D9;">
                        ...
                        <li><strong>Crear carpetas:</strong> Ve a <a href="/admin/informes_congreso/carpetainformescongreso/add/" style="color: #8B5CF6; font-weight: 600;">Carpetas > Agregar carpeta</a>.</li>
                    </ul>
                </div>
            '''
        }),
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == "carpeta":
            ...
            if obj_id:
                try:
                    documento = DocumentoInformesCongreso.objects.get(pk=obj_id)
                    kwargs["queryset"] = CarpetaInformesCongreso.objects.filter(
                        numeral=documento.numeral
                    ).select_related('padre')
                except DocumentoInformesCongreso.DoesNotExist:
                    kwargs["queryset"] = CarpetaInformesCongreso.objects.none()
            else:
        ...
        if 'carpeta' in form.base_fields:
            ...
            if obj and obj.numeral:
                form.base_fields['carpeta'].help_text = f'''
                    <span style="color: #6D28D9; font-weight: 600;">📁 Carpetas del Numeral {obj.numeral.codigo}</span><br>
    ...
    def numeral_link(self, obj):
        url = reverse('admin:informes_congreso_numeralinformescongreso_change', args=[obj.numeral.pk])
        return format_html('<a href="{}" style="color: #8B5CF6; font-weight: 600;">{}</a>', url, obj.numeral.codigo)
    numeral_link.short_description = 'Numeral'
    
    def carpeta_ruta(self, obj):
        if obj.carpeta:
            return format_html(
                '<span style="color: #6D28D9; font-size: 12px; font-weight: 600;">📁 {}</span>',
                obj.carpeta.get_ruta_completa()
            )
        return format_html(
            '<span style="color: #8B5CF6; font-size: 12px; font-weight: 600; '
            'background: #EDE9FE; padding: 2px 8px; border-radius: 4px;">📋 Raíz del Numeral</span>'
        )
    ...
    def acciones_rapidas(self, obj):
        ...
        editar_url = reverse('admin:informes_congreso_documentoinformescongreso_change', args=[obj.pk])
        eliminar_url = reverse('admin:informes_congreso_documentoinformescongreso_delete', args=[obj.pk])
        ...
        <div style="display: flex; gap: 4px; flex-wrap: wrap;">
            <a href="{editar_url}" 
               style="display: inline-flex; align-items: center; padding: 4px 8px; 
                      background: #8B5CF6; color: white; border-radius: 4px; 
