...
class NumeralInformesCongreso(models.Model):
    """
    Numerales específicos de Informes al Congreso
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = slugify(f"informes-congreso-{self.codigo}")
        if self.orden == 0:
            # Orden alfabético: A=1, B=2, etc.
            try:
                self.orden = ord(self.codigo.upper()) - ord('A') + 1
            except:
                self.orden = 999
    ...
    def get_absolute_url(self):
        return reverse('informes_congreso:numeral_detail', kwargs={'slug': self.slug})
...
class CarpetaInformesCongreso(models.Model):
    """
    Estructura jerárquica de carpetas para Informes al Congreso
    """
...
def path_documento_informes_congreso(instance, filename):
    """
    Genera la ruta para documentos de Informes al Congreso
    """
    nombre_limpio = filename.replace(' ', '_')
    ruta_base = f'informes_congreso/numeral_{instance.numeral.codigo}/'
...
class DocumentoInformesCongreso(models.Model):
    """
    Documentos de Informes al Congreso
    """
    ...
    archivo = models.FileField(
        upload_to=path_documento_informes_congreso,
